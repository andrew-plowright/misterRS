...
rts = R6::R6Class(
  ...
  public = list(
    ...
    # This gets overridden in rts and vts
...
vts = R6::R6Class(
  ...
  public = list(
    ...
    fields = function(){
      con <- self$temp_con()
      return(DBI::dbListFields(con, self$geom_layer))
    },
    ...
    #'
    #' NOTE 2: you are GETTING RID of this function, or at least replacing it with 'write_geom_tile', which will save geometry as a SHP file.
    #' You can then re-use the code here for a function that will append all that geometry to the geopackage
    ...
    append_geom = function(data, tile_name){
      ...
      if(nrow(data) > 0){
        ...
        gpkg_fields <- setdiff(DBI::dbListFields(con, self$geom_layer), "fid")
      ...
      DBI::dbExecute(con, sprintf("UPDATE tile_reg SET %s = TRUE where tile_name = '%s'", "geom", tile_name))
    ...
    update_data = function(data, tile_name, attribute, overwrite = FALSE){
      ...
      if(nrow(data) > 0){
        ...
        DBI::dbWriteTable(con, name = "temp_data", value = data, overwrite = TRUE)

        # Update data
        DBI::dbExecute(con, "SELECT load_extension('mod_spatialite')")
        sql <- sprintf("UPDATE %1$s SET (%4$s) = (SELECT %5$s FROM temp_data WHERE temp_data.%2$s = %1s.%2$s) WHERE tile_name = '%3$s'",
      ...
      # Update tile registry
      DBI::dbExecute(con, sprintf("UPDATE tile_reg SET %s = TRUE where tile_name = '%s'", attribute, tile_name))
    ...
    complete = function(attribute, tile_names = NULL){
      ...
      if(is.null(tile_names)){
        ...
        tile_names <- DBI::dbGetQuery(con, "SELECT tile_name FROM tile_reg")[,"tile_name"]
      }
    ...
    #' @description Get folder path for tiles.
    temp_tile_dir = function(dir_name){
      return(file.path(self$dir, paste0("temp_", dir_name)))
    },

    #' @description Get folder path for tiles.
    temp_tile_dir_create = function(dir_name){
      temp_dir <- self$temp_tile_dir(dir_name)
      if(!dir.exists(temp_dir)) dir.create(temp_dir)
    },

    #' @description Get file path for a given tile.
    #' @param tile_name name of tile
    temp_tile_path = function(tile_name, dir_name, ext){

      return(setNames(R.utils::getAbsolutePath(file.path(self$temp_tile_dir(dir_name), paste0(tile_name, ".", ext))), tile_name))
    },
    ...
    temp_absorb_queue = function(dir_name, attribute, ext){

      temp_dir <- self$temp_tile_dir(dir_name)

      # Get temp files
      temp_tile_paths <- list.files(temp_dir, full.names = TRUE, pattern = paste0("\\.",ext,"$"))
      names(temp_tile_paths) <- tools::file_path_sans_ext(basename(temp_tile_paths))

      # Subset those that don't exist
      tiles_exist <- self$has_tiles(names(temp_tile_paths), attribute)
      temp_tile_paths <- temp_tile_paths[!tiles_exist]

      cat("Absorbing temp files:", length(temp_tile_paths), "\n")

      return(temp_tile_paths)
    }
